...
function create_report($mantenedora, $provincia, $mes, $anno, $state) {
...
$objXLS->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1, 12);
...
$objXLS->getActiveSheet()->getHeaderFooter()->setOddFooter($objXLS->getProperties()->getTitle() . '&RPage &P of &N');
...
$rutaReport = 'almacen/' . $mantenedora . "/" . clean_name(get_name_province($provincia)) . "/";

$mesText = $meses[$mes-1];
$provinciaText = obtenerProvincia($provincia);
$nombreInforme = $anno . $mes . date(d) . date(H) . date(i) . "-" . get_nombre_consonate(clean_name($mantenedora)) . "-" . get_nombre_consonate(clean_name(get_name_province($provincia))) . "-" . $state . ".xls";
...
$qIntervenciones .= "CONCAT(UPPER(u.Unidad), ' ', UPPER(u.TipoUnidad)) AS Unidad, ";
...
$qIntervenciones .= "CONCAT(UPPER(ud.Unidad), ' ', UPPER(ud.TipoUnidad)) AS UnidadDestino, ";
...
$objWriter->save($rutaReport . $nombreInforme);


 $patron3 = "/" . $nombreInforme ."/";
 	// Si hay informe es que se ha creado
	if(preg_match($patron3, $nombreInforme)) {

		// Buscamos en todos los archivos del directorio
		$patron_glob2 = $rutaReport  . "*.xls";

		// En cada archivo  comprobar si es de la fecha actual y si es distinto al archivo que se acaba de crear
		foreach (glob($patron_glob2) as $ArchivosSobrantes) {
			if(preg_match("/^(" . $anno . $mes . ")/", str_replace($rutaReport, "",$ArchivosSobrantes)) && ($ArchivosSobrantes != $rutaReport . $nombreInforme)) {
				unlink($ArchivosSobrantes);
			} 
		}
